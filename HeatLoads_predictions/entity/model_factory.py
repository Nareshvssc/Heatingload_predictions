...
def evaluate_regression_model(model_list: list, X_train:np.ndarray, y_train:np.ndarray, X_test:np.ndarray, y_test:np.ndarray, base_accuracy:float=0.6) -> MetricInfoArtifact:
    ...
    try:
        ...
        for model in model_list:
            ...
            logging.info(f"Train Score\t\t Test Score\t\t Model_accuracy")
            ...
            logging.info(f"base accuracy from last model training is : [{base_accuracy}]")
        ...
        logging.info(f"returning metric info artifact after regression testing: [{metric_info_artifact}]")
...
class ModelFactory:
    ...
    def execute_grid_search_operation(self, initialized_model: InitializedModelDetail, input_feature,
        ...
        try:
            ...
            #logging.info(output_feature)
           # logging.info("----input feature--")
            #logging.info(f"{input_feature}")
    ...
    def initiate_best_parameter_search_for_initialized_models(self,
                                                              initialized_model_lists: List[InitializedModelDetail],
                                                              input_feature,
                                                              output_feature) -> List[GridSearchedBestModel]:

        try:
            self.grid_searched_best_model_list = []
            for initialized_model_list in initialized_model_lists:
    ...
    def get_best_model(self, X, y,base_accuracy=0.6) -> BestModel:
        try:
            ...
            grid_searched_best_model_list = self.initiate_best_parameter_search_for_initialized_models(
                initialized_model_lists=initialized_model_list,
                input_feature=X,
                output_feature=y
            )
